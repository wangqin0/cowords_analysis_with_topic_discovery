...
dataset = 'tianya_posts_test_set_100.txt'
...
catd.save_obj(word_net, 'original_' + dataset.split('.')[0])
...
# rebuild word net
selected_words_set = catd.collect_mutual_words_to_set_from_dir(
    os.path.join('data', 'selected_words'))
# add selected_words_set parameter
cut_corpus_new = catd.word_cut(corpus, stop_words_set, selected_words_set)
...
catd.save_obj(word_net_with_selection, 'reduced_' + dataset.split('.')[0])
print()
